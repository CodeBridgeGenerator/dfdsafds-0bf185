...
import customerFakerFactory from "./customerFakerFactory";
...
const CustomerFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = customerFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("customer").create(elem));
...
export default connect(mapState, mapDispatch)(CustomerFakerDialogComponent);
